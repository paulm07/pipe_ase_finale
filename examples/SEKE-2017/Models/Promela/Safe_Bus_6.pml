  ...
  for (h in place_Cable_free) {
    for (y in place_Cpt2) {
      for (k in place_FMCb) {
        for (ix in place_AMC) {
          if
          :: (y.field1 == ix.field2) -> enableAll(); fire_C_provide_0(); goto endfire;
          :: else -> skip;
          fi
...
inline is_enabled_I_reemit() {
  for (ir in place_wait_ack) {
    for (k in place_T_out) {
      if
      :: (true) -> enableAll(); fire_I_reemit_0(); goto endfire;
      :: else -> skip;
      fi
...
inline is_enabled_loss_a() {
  for (h in place_ACK) {
    for (k in place_R_out) {
      if
      :: (true) -> enableAll(); fire_loss_a_0(); goto endfire;
      :: else -> skip;
      fi
...
inline is_enabled_loss_m() {
  for (r in place_MSG) {
    for (k in place_R_out) {
      if
      :: (true) -> enableAll(); fire_loss_m_0(); goto endfire;
      :: else -> skip;
      fi
...
inline is_enabled_I_free() {
  for (h in place_ACK) {
    for (ir in place_wait_ack) {
      for (k in place_FMCb) {
        if
        :: (true) -> enableAll(); fire_I_free_0(); goto endfire;
        :: else -> skip;
        fi
