...
byte ID_AuthB = 32;
byte ID_leaveB = 64;
byte ID_timeoutA = 128;
byte ID_timeoutB = 256;
byte ID_decide = 512;
byte ID_alternate = 1024;
byte allEnabled = 2047;
byte enabledAny = 2047;
...
inline is_enabled_T10() {
  for (s in place_range) {
    for (n in place_capacity) {
      if
      :: (n.field1 == 5) -> enableAll(); fire_T10_0(); goto endfire;
      :: else -> skip;
      fi
...
inline is_enabled_decide() {
  for (s in place_choice) {
    for (cpt in place_computer) {
      if
      :: (cpt.field1 < 2) -> enableAll(); fire_decide_0(); goto endfire;
      :: else -> skip;
      fi
...
inline is_enabled_alternate() {
  for (s in place_choice) {
    for (cpt in place_computer) {
      if
      :: (cpt.field1 == 2 && s.field1 == 1) -> enableAll(); fire_alternate_0(); goto endfire;
      :: (cpt.field1 == 2 && s.field1 == 2) -> enableAll(); fire_alternate_1(); goto endfire;
      :: else -> skip;
      fi
