...
public class SyntaxTreeCrawler implements Visitor {
	...
	public ArrayList<String> mUndefinedVariables = new ArrayList<>(); //record all vars in formula that has not yet defined in arc var
	...
	private HashSet<String> mIntegralChangeVariables = new HashSet<>();
	boolean debug = false;
...
//		}
	...
	public void visit(VariableTerm elem) {
		...
		if (!mIntegralChangeVariables.contains(elem.v.key)) {
	...
	public void visit(IntegralTerm pTerm) {
		mIntegralChangeVariables.add(pTerm.getChangeVariable());

		pTerm.getExpression().accept(this);
	}
}
