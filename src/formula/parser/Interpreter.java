...
import formula.absyntree.Power;
import formula.absyntree.*;
import formula.evaluator.ArithmeticProvider;
import formula.evaluator.Evaluator;
...
public class Interpreter implements Visitor {
    ...
    private static final DataType INTEGRAL_VARIABLE_DATATYPE = new DataType("global_type", new String[]{BasicType.TYPES[BasicType.NUMBER]}, false, null);
    private static final Token GLOBAL_TIME_TOKEN = new Token(INTEGRAL_VARIABLE_DATATYPE);
    static {
        GLOBAL_TIME_TOKEN.add(new BasicType[]{new BasicType(BasicType.NUMBER)});
    }
...
//	}
    ...
    private Object valueOfTerm(final Term pTerm) throws Exception {
        Object value = null;
        if (pTerm instanceof SingleValuedExp) {
            value = ((SingleValuedExp) pTerm).value;
        }
        else if (pTerm instanceof ConstantTerm) {
            value = ((ConstantTerm) pTerm).c.value;
        } else if (pTerm instanceof VariableTerm) {
            Variable variable = ((VariableTerm) pTerm).v;
//            Object object = symTable.lookup(variable.key);
//            //Fix-Me:: proper inheritence to improve the following statement
//            int index = variable instanceof IndexVariable ? ((IndexVariable) variable).index -1 : -1;
//            if (object instanceof Token) {
//                Token token = (Token) symTable.lookup(variable.key);
//                value = index>=0 ? token.Tlist.get(index).getValue() : token.Tlist.get(0);
//            }
//            else if (object instanceof abToken) {
//                List token = ((abToken) object).listToken;
//                value = index>=0 ? token.get(index) : token;
//            }
...
//		elem.value = Integer.parseInt(elem.num.n);
    ...
    public void visit(Power elem) {
        ...
        try {
            ...
            errorMsg.error(elem.pos, "Power:: Invalid value - " + iae.getMessage());
        } catch (Exception e) {
            errorMsg.error(elem.pos, "Power::Tree type mismatch! " + e.getMessage());
        }

        elem.value = leftValue.pow(rightValue.intValue());
    }
...
//			newsetvar ="";
    ...
    public void visit(FunctionExp pTerm) {
          ...
          if (term instanceof VariableTerm) {
              term.accept(this);
              Object object = symTable.lookup(((VariableTerm)term).v.key);
              if (object instanceof abToken) {
                  pTerm.value = new BigDecimal(((abToken) object).listToken.size());
              }
          }
      }
      else {
...
//        }
    ...
    @Override
    public void visit(IntegralTerm pTerm) {
        updateChangeVariableToken("\u03c4", GlobalClock.getInstance().getClockState().next);

        pTerm.getLowerLimit().accept(this);
        pTerm.getUpperLimit().accept(this);

        double lower = pTerm.getLowerLimit().value.doubleValue();
        double upper = pTerm.getUpperLimit().value.doubleValue();

        SimpsonIntegrator simpsonIntegrator = new SimpsonIntegrator();
        UnivariateFunction function = new InterpretedIntegralFunction(pTerm);;
        double value = simpsonIntegrator.integrate(64, function, lower, upper);
        pTerm.value = new BigDecimal(String.valueOf(value));
    }

    private void updateChangeVariableToken(final String pChangeVariable, final double pValue) {
        Object object = symTable.lookup(pChangeVariable);
        if (object == null) {
            Token token = new Token(INTEGRAL_VARIABLE_DATATYPE);
            token.Tlist.add(new BasicType(BasicType.NUMBER));
            symTable.update(pChangeVariable, token, Symbol.TYPE.SINGLE);
            object = token;
        }

        ((Token)object).Tlist.get(0).setValue(String.valueOf(pValue));
    }

    private class InterpretedIntegralFunction implements UnivariateFunction {
        private final IntegralTermVisitor mVisitor;

        public InterpretedIntegralFunction(final IntegralTerm pIntegralTerm) {
            mVisitor = new IntegralTermVisitor(pIntegralTerm);
        }

        @Override
        public double value(double v) {
            return mVisitor.calculate(v);
        }
    }

    private class IntegralTermVisitor implements Visitor{

        private final IntegralTerm mIntegralTerm;
        private final HashMap<String, BigDecimal> mValuesMap = new HashMap<>();

        public IntegralTermVisitor(final IntegralTerm pIntegralTerm) {
            mIntegralTerm = pIntegralTerm;
        }

        double calculate(final double pValue) {
            updateChangeVariableToken(mIntegralTerm.getChangeVariable(), pValue);
            mValuesMap.clear();
            mIntegralTerm.getExpression().accept(this);
            return mValuesMap.get(mIntegralTerm.getExpression().identityToString()).doubleValue();
        }

        @Override
        public void visit(ConstantTerm elem) {
            if (elem.c instanceof NumConstant) {
                mValuesMap.put(elem.identityToString(), (BigDecimal) ((NumConstant)elem.c).value);
            }
        }
        ...
        public void visit(VariableTerm elem) {
            try {
                mValuesMap.put(elem.identityToString(), numberValueOfTerm(elem));
            } catch (Exception e) {
        ...
        @Override
        public void visit(Div elem) {
            doArithmeticAndUpdateElement(elem, ArithmeticProvider::divide);
        }

        private void doArithmeticAndUpdateElement(ArithExp elem, Evaluator<BigDecimal> pEvaluator) {
            elem.t1.accept(this);
            if (elem.t2 != null) {
                elem.t2.accept(this);
            }
            ...
            BigDecimal rightValue = BigDecimal.ZERO;
            try {
                leftValue = determineValueOfTerm(elem.t1);
                rightValue = elem.t2 == null? null : determineValueOfTerm(elem.t2);
                mValuesMap.put(elem.identityToString(), pEvaluator.evaluate(leftValue, rightValue));
            } catch (Exception e) {
                System.out.println(String.format("left:%f right:%f operation:%s", leftValue.doubleValue(), rightValue.doubleValue(), pEvaluator.toString()));
                throw new RuntimeException(e);
            }
        }

        private BigDecimal determineValueOfTerm(final Term pTerm) {
            if (pTerm instanceof ConstantTerm) {
                return (BigDecimal) ((ConstantTerm)pTerm).c.value;
            }
            else if (pTerm instanceof VariableTerm) {
                Variable variable = ((VariableTerm)pTerm).v;
                Token token = (Token) symTable.lookup(variable.key);
                int index = variable instanceof IndexVariable ? ((IndexVariable) variable).index - 1 : 0;
                return (BigDecimal) token.Tlist.get(index).getValue();
            }

            return mValuesMap.get(pTerm.identityToString());
        }

        @Override
        public void visit(Minus elem) {
            doArithmeticAndUpdateElement(elem, ArithmeticProvider::subtract);
        }

        @Override
        public void visit(Mod elem) {
            doArithmeticAndUpdateElement(elem, ArithmeticProvider::negate);
        }

        @Override
        public void visit(Mul elem) {
            doArithmeticAndUpdateElement(elem, ArithmeticProvider::multiply);
        }
        ...
        @Override
        public void visit(Plus elem) {
            doArithmeticAndUpdateElement(elem, ArithmeticProvider::add);
        }

        @Override
        public void visit(Power elem) {
            doArithmeticAndUpdateElement(elem, ArithmeticProvider::power);
        }
        ...
        public void visit(AExp elem) {
            ...
            String key = elem.identityToString();
            BigDecimal value = mValuesMap.get(elem.ae.identityToString());
        ...
        public void visit(ExpTerm elem) {
            try {
                elem.e.accept(this);
                mValuesMap.put(elem.identityToString(), mValuesMap.get(elem.e.identityToString()));
            } catch (RuntimeException ex) {
                System.out.println(String.format("Exception to process formula: %s. Element %s could not be evaluated properly", iTransition.getFormula(), elem.e));
                throw ex;
            }
        ...
        public void visit(FunctionExp pTerm) {
            ...
            for (Term term : pTerm.getArgumentTerms()) {
                term.accept(this);
                try {
                    arguments[index++] = determineValueOfTerm(term);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            }
            Object value = mFunctionHandler.invoke(functionName, arguments);
            if (value instanceof BigDecimal) {
                mValuesMap.put(pTerm.identityToString(), ((BigDecimal)value));
            }
        }

        @Override
        public void visit(IntegralTerm pTerm) {
//            TODO:: To support multi variable integration this need to be implemented
        }
    }
}
