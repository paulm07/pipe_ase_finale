  ...
  */    //not useful 12/17/15
  ...
  private void defineIsEnabledFunc() {
    ...
    for (int transNo = 0; transNo < transSize; transNo++) {
      ...
      List<Place> inputPlaces = trans[transNo].getPlaceInList();
      ...
      Parse p = null;
      try {
        p = new Parse(formula, errorMsg);
      } catch (Exception e) {
        throw new RuntimeException(String.format("Error during parsing formula \"%s\" from transition %s", formula, trans[transNo].getName()));
      }
      Sentence s = p.absyn;
...
//			s.accept(new Printer());
  ...
  private void defineFireFunc() {
    ...
    for (int transNo = 0; transNo < transSize; transNo++) {
      ...
      String formula = trans[transNo].getFormula();
      ErrorMsg errorMsg = new ErrorMsg(formula);
      Sentence s = FormulaUtil.parseFormula(trans[transNo], errorMsg);
//			System.out.println(trans[transNo].getName());
      s.accept(new Formula2Promela(errorMsg, trans[transNo], 0));
      sPromela += s.strPost;

      List<Place> otPlaces = trans[transNo].getPlaceOutList();
