...
public class PlaceTypePanel extends JPanel {
  ...
  private void initState() {
    if (dataType != null) {
      int index = mDatatypePoolModel.indexOf(dataType.getName());
  ...
  private void initModel() {
    ...
    pnmlData.getDataTypes().forEach(d -> mDatatypePoolModel.addElement(d.getName()));
...
//	      mDatatypePoolUI.setSize(TypeTextField.getWidth() ,TypeTextField.getHeight());
  ...
  private void insertSelectedDataTypeToDefinition() {
    String selected = mDatatypePoolUI.getSelectedValue().toString();
    DataType selectedDataType = pnmlData.getDataType(selected);
  ...
  private void loadButtonHandler(ActionEvent evt) {
    if (loadButton.getText().equals("Load")) {
      if (!this.mDatatypePoolUI.isSelectionEmpty()) {
        dataType = pnmlData.getDataType(mDatatypePoolUI.getSelectedValue().toString());
...
//			   this.mDatatypePoolUI.setEnabled(true);
  ...
  private boolean createOrUpdateDataType(final boolean pShouldCreate) {
    ...
    if (pShouldCreate) {
      ...
      if (mDatatypePoolModel.indexOf(dataTypeName) != -1) {
        showWarning("Name already exists");
        return false;
      }

      DataType dataType = new DataType(dataTypeName, types, powcheckbox.isSelected(), null);
//      group.add(dataType);
      pnmlData.addDataType(dataType);
      mDatatypePoolModel.addElement(dataType.getName());
      this.dataType = dataType;
    }
    else {
//      Fix-Me: now since the datatypes are shared among places, modifying should not be allowed unless safety check.
// Until then it is better to disallow modification
