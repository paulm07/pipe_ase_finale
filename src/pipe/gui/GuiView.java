...
public class GuiView 
   ...
   class MouseHandler extends MouseInputAdapter {
      ...
      private PlaceTransitionObject newPlace(Point p, boolean isContinuous){
         p = adjustPoint(p, view.getZoom());
         Place place = new Place(Grid.getModifiedX(p.x), Grid.getModifiedY(p.y));
         place.setIsContinuous(isContinuous);
         model.addPetriNetObject(place);
         view.addNewPetriNetObject(place);

         return place;
      }

      
      private PlaceTransitionObject newTransition(Point p, int mode){
         p = adjustPoint(p, view.getZoom());
         
         Transition transition = new Transition(Grid.getModifiedX(p.x), Grid.getModifiedY(p.y));
         transition.setTimed(mode == Pipe.TIMEDTRANS);
         transition.setIsContinuous(mode == Pipe.CONT_TRANS);
         model.addPetriNetObject(transition);
         view.addNewPetriNetObject(transition);
         return transition;
      }
...
//       }
      ...
      public void mousePressed(MouseEvent e){
         ...
         if (SwingUtilities.isLeftMouseButton(e)) {
            ...
            switch (mode){
               case Pipe.PLACE:
               case Pipe.CONT_PLACE:
                  pnObject = newPlace(e.getPoint(), mode == Pipe.CONT_PLACE);
                  getUndoManager().addNewEdit(new AddPetriNetObjectEdit(pnObject, view, model));
                  if (e.isControlDown()) {
               ...
               case Pipe.CONT_TRANS:
                  pnObject = newTransition(e.getPoint(), mode);
               ...
               case Pipe.FAST_PLACE:
                  if (e.isMetaDown() || metaDown) { // provisional
                     ...
                     createPTO = newPlace(e.getPoint(), mode == Pipe.CONT_PLACE);
                     getUndoManager().addNewEdit(
               ...
               case Pipe.FAST_TRANSITION:
                  if (e.isMetaDown() || metaDown) { // provisional
                     ...
                     createPTO = newTransition(e.getPoint(), mode);
                     getUndoManager().addNewEdit(
                             new AddPetriNetObjectEdit(createPTO, view, model));
                     pnObject.getMouseListeners()[0].mouseReleased(e);
                     if (e.isControlDown()){
