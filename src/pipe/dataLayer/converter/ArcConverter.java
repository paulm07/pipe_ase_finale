...
//	        System.out.println("PNMLDATA: targetInput is not null");
    ...
    String dataTypeName = inputArcElement.getAttribute("datatype");
    if (tempArc.getDataType() == null) {
      pDataLayer.addUnresolvedDataType(dataTypeName, tempArc);
    }
    ...
    String container = inputArcElement.getAttribute("container");
    if (container != null && !container.isEmpty()) {
      tempArc.setContainerNet(container);
    }
  ...
  private Element createArcElement(Arc inputArc, Document document) {
    ...
    if (inputArc != null) {
      ...
      if (datatype != null) {
        arcElement.setAttribute("datatype", datatype.getName());
      }
      if (inputArc.getContainerNet() != null && inputArc.getContainerNet().length() > 0) {
        arcElement.setAttribute("container", inputArc.getContainerNet());
      }
