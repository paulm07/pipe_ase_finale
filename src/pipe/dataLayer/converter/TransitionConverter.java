...
public class TransitionConverter extends DefaultPTNObjectConverter<Transition> {
    ...
    private Transition createTransition(final Element element, final DataLayer pDataLayer){
        ...
        String continuous = element.getAttribute("continuous");
      ...
      timedTransition = nameTimed.length() != 0 && nameTimed.length() != 5;
        ...
        Transition transition =
                new Transition(positionXInput, positionYInput,
                        ...
                        priority,
                        formula);
      transition.setIsContinuous(Boolean.valueOf(continuous));
      String container = element.getAttribute("container");
      if (container != null && !container.isEmpty()) {
        transition.setContainerNet(container);
      }
  ...
  private Element createTransitionElement(Transition inputTransition,
    ...
    if (inputTransition != null ) {
      ...
      transitionElement.setAttribute("continuous", String.valueOf(inputTransition.isContinuous()));
      if (inputTransition.getContainerNet() != null && inputTransition.getContainerNet().length() > 0) {
        transitionElement.setAttribute("container", inputTransition.getContainerNet());
      }
    }
