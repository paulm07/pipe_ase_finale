 ...
 */
public class Place 
        extends PlaceTransitionObject implements DataTypeAware {
  ...
  */
   ...
   private static Ellipse2D.Double place1 = new Ellipse2D.Double(5, 5, 20, 20);
   private static Shape proximityPlace = (new BasicStroke(Pipe.PLACE_TRANSITION_PROXIMITY_RADIUS)).createStrokedShape(place);
...
//         boolean isAllowed = isContinuous() ? someArc.getSource().isContinuous() : true;
  ...
  //# Added Abhinav  8/19/2015
   ...
   {
      if (isContinuous()) {
         boolean hasNumber = dt.getTypes().stream().anyMatch(s -> s.equals(BasicType.TYPES[BasicType.NUMBER]));
         if (!hasNumber) {
            throw new IllegalArgumentException("A continuous place must have a number field in the data type.");
         }
      }
	   dataType = dt;
	   token.definetype(dt);
